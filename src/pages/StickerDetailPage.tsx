      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div
            ...
            style={{ background: 'linear-gradient(135deg, #DEF1FF 0%, #9ED4FB33 100%)', minHeight: 320 }}
          >
            <img
              src={allImages[activeImg]}
              alt={sticker.name}
              className="max-h-72 max-w-full object-contain mix-blend-multiply drop-shadow-xl transition-all duration-300"
              style={{ padding: '24px' }}
            />
            {sticker.featured && (
          ...
          {allImages.length > 1 && (
            <div className="flex gap-2 flex-wrap">
              {allImages.map((img, i) => (
                ...
                >
                  <img src={img} alt={`angle ${i + 1}`} className="w-full h-full object-contain mix-blend-multiply p-1" />
                </button>
              ))}
            </div>
          )}
