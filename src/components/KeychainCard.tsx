...
interface Props { keychain: Keychain; index?: number; }

export default function KeychainCard({ keychain, index = 0 }: Props) {
  ...
  return (
    <div
      ...
      onClick={() => navigate(`/keychain/${keychain.id}`)}
    ...
    >
      ...
      >
        ...
        {keychain.collection && (
          <span className="absolute top-2 right-2 bg-[#2a80b9]/10 text-[#2a80b9] text-xs font-semibold px-2 py-0.5 rounded-full border border-[#9ED4FB]">
            {keychain.collection}
          </span>
        )}
      </div>
      <div className="p-3">
        ...
        <div className="flex items-center justify-between mt-2">
          <span className="font-bold text-base" style={{ color: '#2a80b9' }}>${keychain.price.toFixed(2)}</span>
          <button
            onClick={handleAdd}
